...
However, `Minimult` is still in **beta** because the author had only a little tests on Cortex-M4 with FPU.

# Features

* Task like that of RTOS
...
* Synchronization
  * `idle` and `kick`
    * A task goes into an idle state and other tasks or interrupts wake it up by kicking.
...
* Priority-based dispatching
  ...
  * `dispatch` can be called by users so that timer-based preemption is also possible.
...
You can find a specific board's examples [here](https://github.com/convexbrain/Minimult/tree/master/examples/).
Currently there are very little examples, however.
*/
